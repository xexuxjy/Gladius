...
def diagnose2(file, filedir, gc,demobec,debug=False):
    ...
    for i in range(header.NrOfFiles):
        ...
        if correction != 0:
            print(str(entry.PathHash) + " " + str(correction))
...
class RomSection():
    def __init__(self, name, name2, hash, address, size2, flags, size, checksum, checksum2):
        ...
        if size2 != 0:
            print(self.name + "size2 "+str(size2))
        ...
        if RomMap[i].flags != 0:
            for j in range(len(RomMap)):
                if (i != j) and (RomMap[j].flags == 0) and (RomMap[i].address == RomMap[j].address):
                    if((RomMap[i].new_address != RomMap[j].new_address) or (RomMap[i].size != RomMap[j].size)):
                        print("adjusted : "+RomMap[i].name+"  moved  "+str(RomMap[i].new_address)+" to "+str(RomMap[j].new_address))
                        print("adjusted : "+RomMap[i].name+"  moved  "+str(RomMap[i].size)+" to "+str(RomMap[j].size))
