...
def unpackBecArchive2(file, filedir,demobec,debug=False):
    ...
    for romSection in RomSections:
        ...
        if len(localByteArray) >0 and localByteArray[0] == 0x78 :
            #romSection.FileByteArray2 = zlib.decompress(romSection.FileByteArray)
            localByteArray = zlib.decompress(romSection.FileByteArray)
            
        romSection.FileByteArray = localByteArray       
       
        WriteSectionInFile(romSection.FileByteArray, filedir, romSection.FileName)
...
def createBecArchive(dir, filename, becmap, gc, demobec,ignorechecksum,debug=False):
    ...
    for item in RomMap:
        ...
        if duplicate :
            ...
            if item.storedSize() > 0 :
                ...
                align = getAlignment(currentAddr,FileAlignment)
                currentAddr += align

            else :
                currentAddr += FileAlignment
    ...
    for item in RomMap:
        ...
        #if item.DataOffset != currentPosition:
        #    print("Offset Mismatch "+item.FileName+" , "+str(item.DataOffset)+" , "+str(currentPosition)+" , Diff : "+ str(item.DataOffset - currentPosition)+"  Size : "+str(item.DataSize)+" , Compressed : "+str(item.CompressedSize))
        #else :
        #    print("Offset Match "+item.FileName+" , "+str(item.DataOffset)+" , "+str(currentPosition)+" , Diff : "+ str(item.DataOffset - currentPosition)+"  Size : "+str(item.DataSize)+" , Compressed : "+str(item.CompressedSize))
        ...
        diff = currentPosition + item.storedSize() + 8
...
if __name__ == "__main__":
    ...
    if args.pack:
        ...
        filename = "f:/tmp/gladius-repack-filelist.txt"
